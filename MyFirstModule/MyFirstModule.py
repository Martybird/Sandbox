...
#    logic.run(self.inputSelector.currentNode(), self.outputSelector.currentNode(), imageThreshold, enableScreenshotsFlag)
  ...
  def onApplyButton(self):
    ...
    self.outputLabel.text = str(logic.centerOfMass)
...
#    annotationLogic.CreateSnapShot(name, description, type, 1, imageData)
    ...
    # create a sphere with the first two markups

  def computeSphereParam(self, markupsNode):
    import numpy as np
    centerOfSphere = np.zeros(3)
    pos1 = np.zeros(3)
    pos2 = np.zeros(3)
    markupsNode.GetNthFiducialPosition(0,pos1)
    markupsNode.GetNthFiducialPosition(1,pos2)
    centerOfSphere = (pos1 + pos2) / 2
    radiusOfSphere = np.linalg.norm(pos1 - pos2) / 2

    return centerOfSphere, radiusOfSphere
